...
1.WEB 서버
- Amazon Linux 2 OS로 구성하였다.
- WEB 서버 접속 후 YUM을 통해 Apache를 설치한다.
- Apache만을 단독으로 할 경우 더 이상 작업이 필요 없지만, 
WAS와 연동할 경우 추후 mod_jk 모듈과 빌드 하기 위해 httpd-devel 패키를 설치해준다.
  ...
  - 가용 영역 지정 (Public subnet)
    
  2. 보안 그룹 구성
  - 새 보안 그룹 생성 (프로토콜 : HTTP, 포트 : 80, 소스 : 0,0,0,0)  
  ...
  ELB 생성이 완료 되었다면, 생성된 ELB의 DNS 주소로 웹페이지가 접속 되는지 확인한다.  
----  
...
- Apache 설치 및 테스트가 완료 되었다면, Tomcat을 설치하자
  ...
  + Tomcat 서비스 시작  
    ...
    ELB 생성 (Network Load Balancer)  

    1. 로드 밸런서 구성  
    - 로드 밸런서 이름 설정 
    - 체계 선택 (내부)
    - 리스너 지정 (프로토콜 : TCP, 포트 : 8009)
    - 가용 영역 지정 (Private subnet)
    
    2. 보안 그룹 구성
    - 새 보안 그룹 생성 (프로토콜 : TCP, 포트 : 8009, 소스 : WEB서버 Subnet IP)  
...
- Apache와 Tomcat을 연동 할때에는 mod_jk 라는 Tomcat Cenneter를 사용한다.
   ...
   ![screenshot](/assets/images/3tier/11.png "width:300px;height:100px")
    ...
    해당 링크를 접속 하고 난뒤  
    JK.{{Version}}.tar.gz  파일이 보이면 오른쪽 마우스를 클릭하여 링크를 복사하자
    ![screenshot](/assets/images/3tier/47.png "width:300px;height:100px")  


   해당 작업은 Apache에서 Tomcat과 연동을 해야 되기 때문에 아파치에서 작업한다.

   Wget 명령어를 이용하여 Tomcat Connector를 명령어로 다운 받는다.  
...
<br>
 ...
 `</IfModule>`<br>
  ...
  /etc/httpd/conf/workers.properties 생성<br>
  <br>
  `worker.list=tomcat`  
...
<br>
  이제 WEB1,2 WAS1,2 인스턴스가 구성되었다면 해당 인스턴스들을 기존에 생성했던 ELB 대상그룹에 추가 시켜주고 테스트를 위해 jsp 파일을 만들어서 확인작업을 해보겠습니다.
